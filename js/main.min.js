...
MakeMap.prototype = {
    ...
    !function() {
        ...
        a.each(function() {
            function a() {
                k = new google.maps.LatLngBounds(), $.each(h, function(a, b) {
                    {
                        var c = i.replace("{{ title }}", b.title).replace("{{ content }}", b.content).replace("{{ coords }}", b.coords.join(";")), d = new google.maps.LatLng(b.coords[0], b.coords[1]);
                        new RichMarker({
                            map: j,
                            position: d,
                            draggable: !1,
                    ...
                    k.extend(d);
                }), g.mapObject.on("click", ".js-marker-close", function(a) {
                    a.stopPropagation(), e();
                });
            }
            function b(a, b, c) {
                var d = Math.pow(2, j.getZoom()), e = (new google.maps.LatLng(j.getBounds().getNorthEast().lat(), j.getBounds().getSouthWest().lng()), 
                j.getProjection().fromLatLngToPoint(a)), f = new google.maps.Point(b / d || 0, c / d || 0), g = new google.maps.Point(e.x - f.x, e.y + f.y), h = j.getProjection().fromPointToLatLng(g);
                j.setCenter(h);
            }
            function c() {
                j = g.mapObject.data("map"), l.center = j.getCenter(), l.zoom = j.getZoom(), setTimeout(function() {}, 100), 
                a(), $W.on("resize", function() {
                    var a = j.getCenter();
                    google.maps.event.trigger(j, "resize"), j.panTo(a);
                });
            }
            function d() {
                f.addClass("_show-map"), j && (j.fitBounds(k), j.getZoom() > 17 && (j.setZoom(17), 
                b(j.getCenter(), parseInt(.05 * $W.width()), -parseInt(.1 * $W.height()))));
            }
            function e() {
                f.removeClass("_show-map"), j && (j.panTo(l.center), j.setZoom(l.zoom));
            }
            var f = $(this), g = {
                mapObject: $(".js-contacts-map-object", f),
                mapClose: $(".js-contacts-map-close", f),
                markerTPL: $(".js-contacts-map-markertpl", f),
                infoShowMap: $(".js-contacts-info-show", f)
            }, h = window.G.contacts.points || [], i = g.markerTPL.html(), j = null, k = null, l = {};
            new MakeMap({
                el: g.mapObject,
                zoom: 14
            }).done(c), g.infoShowMap.on("click", function(a) {
                a.preventDefault(), d();
            }), g.mapClose.on("click", e), $D.on("contacts:map:show", d).on("contacts:map:hide", e);
        });
    }();
});